...
public abstract class Jugador implements Comparable<Jugador> {
    ...
    public int compareTo(Jugador otro) {
        return (1000000 * this.getVidas() + this.getSoplidos())
                - (1000000 * otro.getVidas() + otro.getSoplidos());
    }
    ...
    public void quitarUnaVida() {
        if (!estaVivo())
            throw new RuntimeException("No podemos descontar una vida a un jugador que ya no está vivo.");
        setVidas(getVidas() - 1);
    }
